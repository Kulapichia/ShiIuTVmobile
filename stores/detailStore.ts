    ...
    try {
      ...
      if (preferredSource && id) {
        ...
        try {
          ...
          const allFoundResults: SearchResult[] = [];
          const searchPromises = enabledResources.map(async (resource) => {
            try {
              ...
              if (results.length > 0) {
                ...
                allFoundResults.push(...results); // 先收集所有结果
                if (!firstResultFound) {
                  set({ loading: false }); // 立即停止加载动画，提升感知性能
          ...
          // 所有请求完成后，进行一次性的批量UI更新
          if (allFoundResults.length > 0) {
            await processAndSetResults(allFoundResults, true);
            logger.info(`[SUCCESS] Standard search completed, processed ${allFoundResults.length} results in a batch.`);
          } else if (!get().searchResults.length) {
            // 仅在当前也没有任何结果时才设置错误
